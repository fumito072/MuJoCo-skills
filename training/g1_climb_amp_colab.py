# ============================================================================
#  G1 CLIMB via AMP (Adversarial Motion Priors) — Colab/GPU (MJX), SELF-CONTAINED.
#  Paste each "# === CELL N ===" block into its own Colab cell (Runtime->GPU).
#
#  WHY AMP: the floor->platform climb is unsolved (true floor-start 0/20). The
#  first foot-lift from a flat stand needs ACTIVE single-leg balance a KINEMATIC
#  reference can't supply; DeepMimic TRACKING over-constrains (deviating to
#  balance is punished). AMP instead trains a discriminator that rewards motion
#  whose STYLE matches the reference distribution, paired with a task reward —
#  leaving the policy free to find its own balance. Verified end-to-end on local
#  CPU MJX (env, expert<->robot feature match, discriminator, training loop,
#  disconnect-safe checkpoint/resume). CELL 4 is the HONEST true-floor eval.
# ============================================================================
...
import importlib
for mod in ("mujoco", "mujoco.mjx", "jax", "brax", "mujoco_playground", "flax", "optax"):
...
# === CELL 2 — Drive + embedded reference + AMP blocks + env + expert dataset ===
import os, functools, time, pickle, base64, io
import numpy as np
import jax, jax.numpy as jp, optax, flax, flax.linen as nn
import mujoco
...
REF_LEGS = jp.asarray(_ref["legs"], jp.float32)
REF_BASE = jp.asarray(_ref["base"], jp.float32)
REF_N = int(REF_LEGS.shape[0]); REF_YAW = float(_ref["yaw"]); REF_DT = float(_ref["dt"])
print("reference:", REF_N, "frames")

# ---- AMP building blocks (discriminator, LSGAN+R1 loss, style reward, expert) ----
...
# ---- the climb env (G1ClimbAMP): task reward only; emits info['amp_obs'] ----
...
AMP_EXPERT, _ef = build_expert_pairs(
    env._amp_features, env.mj_model, env.mjx_model, REF_LEGS, REF_BASE, REF_YAW, REF_DT)
print("env + expert ready:", AMP_EXPERT.shape)


# === CELL 3 — AMP TRAIN (disconnect-safe pickle checkpoint + auto-resume) ===
...
def amp_train(environment, expert_pairs, amp_feat_dim, *,
    ...
    disc = Discriminator(hidden=disc_hidden)
    ...
    def disc_update(disc_params, disc_opt_state, amp_norm, policy_pairs_flat, key):
        ...
        (loss, metrics), g = jax.value_and_grad(
            lambda p: disc_loss(disc_apply, p, exp, pol, grad_pen), has_aux=True)(disc_params)
    ...
    def training_step(state: AMPState, env_state, key):
        ...
        style = style_reward(disc_apply, state.disc_params, normalize(flat, state.amp_norm))
...
def progress(step, m):
    print(f"[{datetime.now():%H:%M:%S}] step {step:>11,}  style {m.get('train/style_mean',0):.3f}"
          ...
          f"  total_loss {m.get('total_loss',0):7.2f}", flush=True)
...
def true_floor_eval(step, make_policy, state, n=20):
    ...
    for i in range(n):
        ...
        info = dict(st.info)
        for k in ("amp_obs", "amp_obs_prev"): info[k] = env._amp_features(data)
...
make_policy, state = amp_train(
    env, AMP_EXPERT, 30, num_timesteps=300_000_000, episode_length=400,
    num_envs=4096, unroll_length=20, batch_size=256, num_minibatches=32,
    num_updates_per_batch=4, num_evals=60, learning_rate=3e-4,
    w_task=0.5, w_style=0.5, grad_pen=5.0, disc_lr=1e-4,
    ckpt_dir=CKPT_DIR, progress_fn=progress, eval_fn=true_floor_eval, seed=0)
print("DONE.")


# === CELL 4 — TRUE-floor eval of the latest checkpoint (start_z ~0.755) ===
# (true_floor_eval above is also called every eval during training.)
true_floor_eval(int(state.env_steps), make_policy, state, n=20)


# === CELL 5 — save policy params for Mac inference ===
with open(f"{CKPT_DIR}/g1_climb_amp_params.pkl", "wb") as f:
    pickle.dump(jax.tree_util.tree_map(np.asarray,
        (state.obs_norm, state.ppo_params.policy)), f)
print("saved ->", f"{CKPT_DIR}/g1_climb_amp_params.pkl")
